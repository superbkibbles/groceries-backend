...
func (w *Wishlist) AddItem(productID, variationID primitive.ObjectID) (*WishlistItem, error) {
...
func (w *Wishlist) RemoveItemByProduct(productID, variationID primitive.ObjectID) error {
	for i, item := range w.Items {
		if item.ProductID == productID && item.VariationID == variationID {
			lastIndex := len(w.Items) - 1
			w.Items[i] = w.Items[lastIndex]
...
func (w *Wishlist) ContainsProduct(productID, variationID primitive.ObjectID) bool {
	for _, item := range w.Items {
		if item.ProductID == productID && item.VariationID == variationID {
			return true
		}
